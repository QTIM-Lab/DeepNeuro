...
Pull the Segment_GBM Docker container from https://hub.docker.com/r/qtimlab/deepneuro_segment_mets/. Use the command "docker pull qtimlab/deepneuro_segment_mets".

You can then enter a command using the following template to generate a segmentation:

```
nvidia-docker run --rm -v [MOUNTED_DIRECTORY]:/INPUT_DATA qtimlab/deepneuro_segment_mets segment_mets pipeline -T1 <file> -T1POST <file> -FLAIR <file> -output_folder <directory> [-gpu_num <int> -debiased -resampled -registered -save_all_steps -save_preprocessed -segmentation_output <str>]
```
